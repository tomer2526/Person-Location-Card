...
## הגדרה דרך UI
הכרטיס תומך בעורך הוויזואלי של Home Assistant, כולל בחירת רשימת ישויות (מוגבל ל־`sensor` ו־`device_tracker`), בחירת `gps_entity`, והגדרת שם לכל מכשיר. בעורך יוצג גם ה־`friendly_name` ליד ה־entity_id.
